...
로컬 개발 환경에서는 `127.0.0.1` 또는 `localhost`로 접속하면 클라이언트가 기본 API 주소 `http://127.0.0.1:3000`을 자동으로 사용합니다. 다른 서버 주소를 테스트할 때만 브라우저 콘솔에서 아래 값을 설정합니다. 값이 없거나 서버가 실패하면 기존 로컬 저장으로 fallback합니다.
...
## 2026-04-19 계정 UI 및 Mock 결제 테스트 UX 추가
- 수정 목적: Safari 콘솔 없이 회원가입 / 로그인 / 서버 저장 / Mock 결제 흐름을 화면에서 검증
- 수정 파일: `index.html`, `style.css`, `js/api.js`, `js/state.js`, `js/ui.js`, `js/shop.js`, `README.md`
- 해결한 문제
  - 로컬 개발 주소에서는 API 서버 주소를 자동 감지하도록 보강
  - 상단 계정 패널에 회원가입 / 로그인 / 로그아웃 / 서버 불러오기 버튼 추가
  - 계정 상태와 저장 방식을 화면에 표시
  - 로그인 전 웹 상품 버튼은 `로그인 필요`로 표시
  - Mock 결제 실패 시 로그인 필요와 서버 연결 실패를 더 구분해서 안내
- 남은 문제
  - 정식 배포 전 실제 계정 UX, 비밀번호 재설정, 운영용 결제 문구 정리 필요


## 2026-04-19b GitHub Pages 배포 준비
- 수정 목적: 새 Public 저장소 `project-hw-alpha`와 GitHub Pages 루트 배포 기준 정리
- 수정 파일: `README.md`, `DEPLOY_NOTE.txt`, `main.js`, `.env.example`
- 해결한 문제
  - GitHub Pages에서는 정적 클라이언트만 구동하고 서버 API는 별도 호스팅 대상임을 명시
  - 실서버 주소가 없을 때 로컬 저장 기반 플레이가 유지되는 기준 정리
